...
def add_row_into(matrix, source, dest):
    """ adds each element of the row with index source (the source row) to the corresponding element of the row with index dest (the destination row)
        input: the specified 2-D list matrix
    """
    for i in range(len(matrix[0])):
        matrix[dest][i] += matrix[source][i]
    
def add_mult_row_into(matrix, m, source, dest):
    """ adds each element of row source (the source row), multiplied by m, to the corresponding element of row dest (the destination row)
        input: the specified 2-D list matrix
    """
    for i in range(len(matrix[0])):
        matrix[dest][i] += matrix[source][i] * m
